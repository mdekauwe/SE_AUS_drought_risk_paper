...
def main(fname, met_fname, plot_fname=None):
    ...
    axes = [ax1, ax2]
    axes2 = [axx1, axx2]
    ax1.plot(df1.index, df1["theta1"].rolling(window=window).mean(), c=colours[1],
             lw=1.5, ls="-", label="1+2", zorder=1)
    ax1.plot(df1.index, df1["theta2"].rolling(window=window).mean(), c=colours[2],
             lw=1.5, ls="-", label="3+4")
    ax1.plot(df1.index, df1["theta3"].rolling(window=window).mean(), c=colours[0],
             lw=1.5, ls="-", label="5+6")
    ...
    ax2.plot(df1.index, df1["weighted_psi_soil"], c=colours[1],
             lw=1.5, ls="-", label="$\Psi$$_{\mathrm{s,pd-weight}}$")
    ax2.plot(df2.index, df2["psi_leaf"], c=colours[2],
             lw=1.5, ls="-", label="$\Psi$$_{\mathrm{l,midday}}$")
    ax2.plot(df2.index, df2["psi_stem"], c=colours[0],
             lw=1.5, ls="-", label="$\Psi$$_{\mathrm{x,midday}}$")
    #ax3.set_ylim(-5, 0)
    ...
    axx2.set_ylabel("Rainfall (mm d$^{-1}$)", position=(0.5, 1.0))

    axx1.set_yticks([0, 20, 40])
    axx2.set_yticks([0, 20, 40])

    ax1.legend(numpoints=1, loc="best", ncol=3)
    ax2.legend(numpoints=1, ncol=3, loc=(0.2, 0.2))
...
if __name__ == "__main__":
    ...
    plot_fname = "water_potentials.pdf"
    main(fname, met_fname, plot_fname)
