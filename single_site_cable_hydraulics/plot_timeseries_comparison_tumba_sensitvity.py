...
def main(site, met_fname, flux_fname, fname1, fname2, fname3, fname4, fname5,
         fname6, plot_fname=None):
    ...
    df5 = resample_timestep(df5, type="CABLE")

    df6 = read_cable_file(fname6, type="CABLE")
    df6 = resample_timestep(df6, type="CABLE")
    ...
    for a, x, v in zip(axes, axes2, vars):
        ...
        a.plot(df1[v].index.to_pydatetime(), df1[v].rolling(window=5).mean(),
               c=colours[0], lw=1.5, ls="-", label="Default Hydraulics")
        a.plot(df2[v].index.to_pydatetime(), df2[v].rolling(window=5).mean(),
               c=colours[2], lw=1.5, ls="-", label="Sensitivity")
        a.plot(df3[v].index.to_pydatetime(), df3[v].rolling(window=5).mean(),
               c=colours[2], lw=1.5, ls="-")
        a.plot(df4[v].index.to_pydatetime(), df4[v].rolling(window=5).mean(),
               c=colours[2], lw=1.5, ls="-")
        a.plot(df5[v].index.to_pydatetime(), df5[v].rolling(window=5).mean(),
               c=colours[2], lw=1.5, ls="-")
        a.plot(df6[v].index.to_pydatetime(), df6[v].rolling(window=5).mean(),
               c=colours[2], lw=1.5, ls="-")
...
if __name__ == "__main__":
    ...
    #fname1 = "outputs/hydraulics_tumba.nc"
    fname1 = "outputs/hydraulics_tumba_resistivity.nc"
    fname2 = "outputs/hydraulics_tumba_200.nc"
    fname3 = "outputs/hydraulics_tumba_400.nc"
    fname4 = "outputs/hydraulics_tumba_600.nc"
    fname5 = "outputs/hydraulics_tumba_800.nc"
    fname6 = "outputs/hydraulics_tumba_1000.nc"

    odir = "/Users/mdekauwe/Dropbox/Drought_risk_paper/figures/figs"
    plot_fname = os.path.join(odir, "tumba_sensitvity.pdf")
    main(site, met_fname, flux_fname, fname1, fname2, fname3, fname4, fname5,
         fname6, plot_fname=plot_fname)
