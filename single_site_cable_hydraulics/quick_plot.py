...
def main(site, met_fname, flux_fname, fname, plot_fname=None):
    ...
    for a, x, v in zip(axes, axes2, vars):
        if v == "Qle":
            a.plot(df_flx.index.to_pydatetime(), df_flx[v].rolling(window=7).mean(), c=colours[1], lw=2.0,
                ls="-", label="Observations")

        a.plot(df1.index.to_pydatetime(), df1[v].rolling(window=7).mean(), c=colours[0],
...
if __name__ == "__main__":
    ...
    fname = "outputs/original_ozflux.nc"
    main(site, met_fname, flux_fname, fname)
