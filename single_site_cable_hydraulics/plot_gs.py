...
def main(site, met_fname, flux_fname, fname1, fname2, plot_fname=None):
    ...
    ax1.set_ylim(0, 0.2)
    ax2.set_ylim(0, 0.2)
    labels = ["g$_{sw,sun}$ (mol m$^{-2}$ s$^{-1}$)", "g$_{sw,sha}$ (mol m$^{-2}$ s$^{-1}$)"]
...
def read_cable_file(fname, type=None):
    ...
    if type == "CABLE":
        ...
        df['gsw_sun'] = f.variables['gsw_sun'][:,0,0]
        df['gsw_sha'] = f.variables['gsw_sha'][:,0,0]
    elif type == "FLUX":
